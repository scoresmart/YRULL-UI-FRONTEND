...
import { clearMetaAuthError, isMetaTokenExpired, subscribeMetaAuthStatus } from '../lib/metaAuthStatus';
...
export function useWhatsAppIntegration() {
  ...
  const tokenExpired = useSyncExternalStore(subscribeMetaAuthStatus, isMetaTokenExpired, () => false);
  ...
  return {
    ...
    // The backend reports `connected` from cached rows, so it stays true after
    // Meta expires the token. Surface that separately rather than flipping
    // `connected` — cached conversations are still worth showing.
    tokenExpired,
    disconnecting,
    workspaceId,
    chooseNumberState,
