// Single source of truth for the third-party services Yrull can talk to.
//
// Both the Integrations settings page and the automation builder read this list.
// They used to keep separate copies, which drifted: the builder knew about six
// services while settings offered eleven, so a connected Cellcast or Meta CAPI
// could never be picked as an automation step.
...
// Whether the workspace has supplied enough configuration for a service to run.
// Most services are connected by filling in their fields; the OAuth ones
// (WhatsApp, Gmail, Instagram) declare no fields, so any stored value counts.
export function isIntegrationConnected(integration, config) {
  if (!config) return false;
  const fields = integration.fields ?? [];
  const values = fields.length > 0 ? fields.map((f) => config[f.key]) : Object.values(config);
